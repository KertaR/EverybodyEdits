package
{
   ...
   public class Shop
   {
      ...
      {
         if(Global.playerObject != null && Global.playerObject.gems > 0)
         {
            return Global.playerObject.gems;
         }
         return (client != null && client.payVault != null) ? client.payVault.coins : 0;
      }
      ...
      public static function refresh(param1:Function) : void
      {
         ...
         try
         {
            Global.log("Shop.refresh() called, base=" + (base != null));
            if(base != null)
            {
               base.requestRemoteMethod("getShop", function(param1:Message):void
               {
                  Global.log("Shop.refresh() getShop response callback received, msg=" + param1);
                  try
                  {
                     update(param1, false);
                  }
                  ...
                  {
                     Global.log("Shop.refresh() update error: " + eUpd.message);
                  }
                  if(callback != null)
                  {
                     callback();
      ...
      public static function refreshCrewShop(param1:Function) : void
      {
         ...
         try
         {
            if(client != null && client.payVault != null)
            {
      ...
      private static function update(param1:Message = null, param2:Boolean = false) : void
      {
         ...
         Global.log("Shop.update() called, param1 len=" + (param1 ? param1.length : 0));
         refreshed = true;
             ...
             while(_loc5_ < param1.length)
             {
                try
                {
                   ...
                   if(_loc6_.type != null && (!_loc6_.isDevOnly || Config.show_disabled_shopitems))
                   {
                      ...
                      {
                         try { setBitmapData(_loc6_); } catch(e:Error) {}
                      }
                      if(_loc6_.id != "smileypostman")
                      {
                ...
                {
                   Global.log("Shop.update error parsing item at " + _loc5_ + ": " + err.message);
                }
                _loc5_ += 25;
             }
         }
         var hashCount:int = 0;
         for(var k:String in dataidhash) { hashCount++; }
         Global.log("Shop.update finished, dataidhash count=" + hashCount + ", dispatching ShopEvent.UPDATE");
         last_gemamount = gems;
      ...
      public static function setBitmapData(param1:ShopItemData) : BitmapData
      {
         ...
         {
            case "smilies":
               var sItem:Object = ItemManager.getSmileyByPayvaultId(param1.id);
               if(sItem != null && sItem.bmd != null)
               {
                  _loc2_ = sItem.bmd;
                  _loc3_ = _loc2_.width;
                  param1.bitmapsheet_offset = 0;
               }
               else
               {
                  _loc2_ = ItemManager.smileysBMD;
                  _loc3_ = 26;
               }
               _loc4_ = 0;
               break;
            case "auraColors":
             ...
             case "auraShapes":
                ...
                _loc3_ = _loc2_ ? _loc2_.width : 64;
                param1.bitmapsheet_offset = 0;
                break;
             case "worlds":
                _loc2_ = ItemManager.shopWorldsBMD;
                _loc3_ = 194;
                break;
             case "services":
                ...
                break;
             case "npcs":
                _loc2_ = ImageUtils.createNpcImageFromPayVaultId(param1.id);
                _loc3_ = _loc2_ ? _loc2_.width : 16;
                param1.bitmapsheet_offset = 0;
                break;
             case "bricks":
                switch(param1.id)
         ...
         spendEnergy = function(param1:Message):void
         {
            ...
            if(m.getBoolean(0))
            {
             ...
             else
             {
                try
                {
                   var sid:ShopItemData = dataidhash[target];
                   if(sid != null && Global.playerObject && Global.playerObject.itemEnergyProgress)
                   {
                      var prog:int = int(Global.playerObject.itemEnergyProgress[target] || 0);
                      sid.energyUsed = prog;
                   }
                   _energy = Global.playerObject ? Global.playerObject.energy : _energy;
                }
                catch(eUp:Error) {}
                dispatchEvent(new ShopEvent(ShopEvent.UPDATE));
             }
      ...
      private static function handleSetCrewName() : void
      {
         ...
         prompt.savebtn.addEventListener(MouseEvent.CLICK,function(param1:MouseEvent):void
         {
            ...
            if(prompt.inputvar.text.length > 0)
            {
               ...
               cPrompt = new ConfirmPrompt("Are you sure you want to name your crew \"" + crewName + "\"? This cannot be changed afterwards!",true,null,false);
               Global.base.showOnTop(cPrompt);
               cPrompt.btn_yes.addEventListener(MouseEvent.CLICK,function(param1:MouseEvent):void
               {
                  var e:MouseEvent = param1;
      ...
      public static function buyItem(param1:String, param2:Function) : void
      {
         ...
         var paypalargs:Object = {
            ...
            "cpp_header_image":Config.site + "/Images/ee_paypal_logo.png",
            "on0":"UserID",
            "os0":client.connectUserId,
            "cancel_return":Config.site,
            "lc":"US"
         };
         paypalargs["return"] = Config.site + "/thankyou.html";
      ...
      public static function getMoreGems(param1:int = -1) : void
      {
            ...
            TweenMax.to(gs,0.2,{
               ...
               "onComplete":function():void
               {
                  TweenMax.to(gsbg,0.25,{
                     ...
                     "onComplete":function():void
                     {
                        if(Boolean(gs) && Boolean(gs.parent))
                        {
                           gs.parent.removeChild(gs);
                        }
                        if(Boolean(gsbg) && Boolean(gsbg.parent))
                        {
         ...
         else
         {
            ...
            {
               paypalargs["coinamount"] = "575";
               paypalargs["item_name"] = "575 Everybody Edits Gems";
               getProviderUrl("paypal",paypalargs,successcallback);
            });
            gs.buybtn7.addEventListener(MouseEvent.CLICK,function():void
            {
               paypalargs["coinamount"] = "1000";
               paypalargs["item_name"] = "1000 Everybody Edits Gems";
               getProviderUrl("paypal",paypalargs,successcallback);
            });
            hidesuperrewards = new Sprite();
            hidesuperrewards.x = gs.x + 35;
            hidesuperrewards.y = 325;
            hidesuperrewards.graphics.beginFill(16777215);
            hidesuperrewards.graphics.drawRect(0,0,310,72);
            smileygx = new FriendSmiley(ItemManager.smileysBMD);
            smileygx.frame = 0;
      ...
      public static function getProviderUrl(param1:String, param2:Object, param3:Function) : void
      {
         ...
         var _loc4_:String = "https://api.playerio.com/payvault/" + param1 + "/coinsredirect?gameid=" + Config.playerio_game_id + "&connectuserid=" + Global.client.connectUserId + "&connection=secure";
         for(_loc5_ in param2)
         {
            _loc4_ += "&" + _loc5_ + "=" + escape(param2[_loc5_]);
         }
         param3(_loc4_);
      }
